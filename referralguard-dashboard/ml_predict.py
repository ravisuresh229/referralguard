...
def create_features_from_provider(provider_data):
    ...
    # Risk score (composite) - Generate more varied scores
    base_risk = (
    ...
    # Add some randomness and specialty-based variation
    specialty_risk_multiplier = {
        'Cardiology': 1.3,
        ...
        'Hospitalist': 1.1,
        'Certified Registered Nurse Anesthetist (CRNA)': 0.7,
        'General Practice': 1.0,
        'Family Practice': 1.0,
        'Physical Medicine and Rehabilitation': 0.8,
        'Urology': 1.2,
        'Clinical Laboratory': 0.6,
        'Ambulance Service Provider': 1.5,
        'Physical Therapist in Private Practice': 0.5,
        'Interventional Radiology': 1.3,
        'Centralized Flu': 0.4,
        'Hematology-Oncology': 1.4
    }
    
    specialty_multiplier = specialty_risk_multiplier.get(specialty, 1.0)
    features['risk_score'] = base_risk * specialty_multiplier + (hash(provider_name) % 20) * 0.1
    ...
    try:
        ...
        if model is None:
            ...
            for provider in provider_data:
                # Enhanced risk calculation with realistic variation
                market_share = provider.get('marketShare', 0)
                specialty = provider.get('specialty', 'Unknown')
                revenue = provider.get('revenue', 0)
                
                # Base risk calculation
                base_risk = (100 - market_share) * 0.3
                
                # Specialty-based risk adjustment
                specialty_risk_multiplier = {
                    'Cardiology': 1.3, 'Internal Medicine': 1.1, 'Emergency Medicine': 1.2,
                    'Obstetrics & Gynecology': 1.0, 'Pathology': 0.8, 'Radiology': 0.9,
                    'Infectious Disease': 1.4, 'Pulmonary Disease': 1.3, 'Hospitalist': 1.1,
                    'Certified Registered Nurse Anesthetist (CRNA)': 0.7, 'General Practice': 1.0,
                    'Family Practice': 1.0, 'Physical Medicine and Rehabilitation': 0.8,
                    'Urology': 1.2, 'Clinical Laboratory': 0.6, 'Ambulance Service Provider': 1.5,
                    'Physical Therapist in Private Practice': 0.5, 'Interventional Radiology': 1.3,
                    'Centralized Flu': 0.4, 'Hematology-Oncology': 1.4
                }
                
                specialty_multiplier = specialty_risk_multiplier.get(specialty, 1.0)
                risk_score = base_risk * specialty_multiplier
                
                # Add revenue-based risk
                if revenue > 100000:
                    risk_score += 15
                elif revenue > 50000:
                    risk_score += 10
                ...
                risk_score += competition_score * 15
                
                # Add provider name-based variation for uniqueness
                provider_name = provider.get('providerName', 'Unknown Provider')
                name_variation = (hash(provider_name) % 30) - 15  # -15 to +15
                risk_score += name_variation
                
                # Ensure realistic bounds
                risk_score = min(max(risk_score, 15), 95)
                revenue_at_risk = (revenue * risk_score) / 100
